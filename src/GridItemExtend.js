...
type State = {
    ...
    itemSizes: Array<Array>,
};

/**
 * An individual item within a ReactGridLayout.
 */
export default class GridItemExtend extends React.Component {
    ...
    onMouseDownPreventDrag(e) {
        e.stopPropagation();
        e.preventDefault();
    }

    onItemClick = (item) => {
        this.onResizeHandler("onResize")(null, { node: null, size: { left: 0, top: 0, width: item.value[0] * 85, height: item.value[1] * 85 } });
        this.onResizeHandler("onResizeStop")(null, { node: null, size: { left: 0, top: 0, width: item.value[0] * 85, height: item.value[1] * 85 } });
    };

    mixinHeader(child: React.Element<any>) {
        return (
            <div>
                    ...
                    {!isEmpty(this.props.itemSizes) &&
                        <DropdownMenu itemClick={this.onItemClick} className="icon-container" >
                            { this.props.itemSizes.map((item) =>
                                <ListItem key={item.join('x')} name={item.join('x')} value={item}>
                                    <span>{item.join('x')}</span> <Grid size={item} />
                                </ListItem>)
                            }
                        </DropdownMenu>
                    }
